...
const ArchUpdateIndicator = GObject.registerClass(
    ...
    class ArchUpdateIndicator extends Button {
        _init(ext) {
            ...
            this.updateNowMenuItem = new PopupMenu.PopupMenuItem(
                _('Update now')
            );
            this.managerMenuItem = new PopupMenu.PopupMenuItem(
                _('Open package manager')
            );

            // A special "Checking" menu item with a stop button
            this.checkingMenuItem = new PopupMenu.PopupBaseMenuItem({
                reactive: false,
            });
            ...
            this.checkNowMenuItem.connect(
                'activate',
                this._checkUpdates.bind(this)
            );
            this.checkNowMenuItem.connect(
                'activate',
                this._checkAuditUpdates.bind(this)
            );
            this.checkNowMenuItem.connect(
                'activate',
                this._checkAuditFull.bind(this)
            );
            ...
            cancelButton.connect(
                'clicked',
                this._cancelAuditFullCheck.bind(this)
            );
            settingsMenuItem.connect('activate', this._openSettings.bind(this));
            this.updateNowMenuItem.connect(
                'activate',
                this._updateNow.bind(this)
            );
            this.managerMenuItem.connect(
                'activate',
                this._openManager.bind(this)
            );
        ...
        _applySettings() {
            ...
            STRIP_VERSIONS_N = this._settings.get_boolean(
                'strip-versions-in-notification'
            );
        ...
        _checkAutoExpandList() {
            if (
                this.menu.isOpen &&
                UPDATES_PENDING > 0 &&
                UPDATES_PENDING <= AUTO_EXPAND_LIST
            ) {
        ...
        _showChecking(isChecking) {
            if (isChecking == true) {
                this.updateIcon.set_gicon(
                    this._getCustIcon('arch-unknown-symbolic')
                );
        ...
        _updateStatus(updatesCount) {
            ...
            if (updatesCount > 0) {
                // Updates pending
                this.updateIcon.set_gicon(
                    this._getCustIcon('arch-updates-symbolic')
                );
                ...
                if (NOTIFY && UPDATES_PENDING < updatesCount) {
                    if (HOWMUCH > 0) {
                        ...
                        if (HOWMUCH > 1) {
                            ...
                            updateList = this._updateList.filter(
                                function (pkg) {
                        ...
                        if (updateList.length > 0) {
                            ...
                            this._showNotification(
                                __(
                                    'New Arch Linux Update',
                                    'New Arch Linux Updates',
                                    updateList.length
                                ),
                                updateList.join(', ')
                            );
                        ...
                        this._showNotification(
                            ...
                            __(
                                'There is %d update pending',
                                'There are %d updates pending',
                                updatesCount
                            ).format(updatesCount)
                        );
                    }
                ...
                if (updatesCount == -1) {
                    ...
                    // Error
                    this.updateIcon.set_gicon(
                        this._getCustIcon('arch-error-symbolic')
                    );
                    if (
                        this.lastUnknowErrorString.indexOf(
                    ...
                    // Up to date
                    this.updateIcon.set_gicon(
                        this._getCustIcon('arch-uptodate-symbolic')
                    );
        ...
        _updateAuditStatus(auditCount) {
            ...
            if (auditCount > 0) {
                ...
                this._updateSecurityMenuExpander(
                    true,
                    __(
                        '%d vulnerable package (above %s)',
                        '%d vulnerable packages (above %s)',
                        auditCount
                    ).format(auditCount, level)
                );
                AUDIT_FULL_LIST = this._auditFullList;
            } else if (auditCount == -1) {
                this._updateSecurityMenuExpander(
                    false,
                    _('Waiting first audit')
                );
            }
        }
        ...
        _updateMenuExpander(enabled, label) {
                ...
                if (enabled && this._updateList.length > 0) {
                    this._updateList.forEach(item => {
                        if (DISABLE_PARSING) {
                            ...
                            this.menuExpander.menu.box.add_child(
                                this._createPackageLabel(menutext)
                            );
                        } else {
                            ...
                            if (matches == null) {
                                ...
                                let __label = this._createPackageLabel(
                                    matches[1]
                                );
                                if (
                                    this._auditUpdateList.includes(matches[1])
                                ) {
                                    hBox.add_child(
                                        new St.Icon({
                                ...
                                if (!STRIP_VERSIONS) {
                                    hBox.add_child(
                                        new St.Label({
                                            ...
                                            y_align: Clutter.ActorAlign.CENTER,
                                            style_class:
                                                'arch-updates-update-version-from',
                                        })
                                    );
                                    hBox.add_child(
                                        new St.Label({
                                            text: matches[3],
                                            style_class:
                                                'arch-updates-update-version-to',
                                        })
                                    );
                                }
            ...
            if (this.updateNowMenuItem.reactive) {
                this.updateNowMenuItem.remove_style_class_name(
                    'popup-inactive-menu-item'
                );
            } else {
                this.updateNowMenuItem.add_style_class_name(
                    'popup-inactive-menu-item'
                );
            }
            if (this.menuExpander.reactive) {
                this.menuExpander.remove_style_class_name(
                    'popup-inactive-menu-item'
                );
            } else {
                this.menuExpander.add_style_class_name(
                    'popup-inactive-menu-item'
                );
            }
        }
        ...
        _packageInfo(item) {
            ...
            proc.communicate_utf8_async(null, null, (proc, res) => {
                ...
                if (proc.get_successful()) {
                    let m = stdout.match(
                        /^Repository\s+:\s+(\w+).*?^Architecture\s+:\s+(\w+)/ms
                    );
                    if (m !== null) {
                        repo = m[1];
                        arch = m[2];
        ...
        _checkUpdates() {
            ...
            try {
                ...
                this._updateProcess_sourceId = GLib.child_watch_add(
                    0,
                    pid,
                    () => {
        ...
        _checkAuditUpdates() {
            ...
            try {
                ...
                // We will process the output at once when it's done
                this._auditUpdateProcess_sourceId = GLib.child_watch_add(
                    0,
                    pid,
                    () => {
                        this._checkAuditUpdatesRead();
                    }
                );
        ...
        _checkAuditFull() {
            ...
            try {
                ...
                // We will process the output at once when it's done
                this._auditFullProcess_sourceId = GLib.child_watch_add(
                    0,
                    pid,
                    () => {
                        this._checkAuditFullRead();
                    }
                );
        ...
        _cancelAuditCheck() {
            if (this._auditUpdateProcess_pid == null) {
                return;
            }
        ...
        _cancelAuditFullCheck() {
            if (this._auditFullProcess_pid == null) {
                return;
            }
        ...
        _checkAuditUpdatesRead() {
            ...
            do {
                [out, size] =
                    this._auditUpdateProcess_stream.read_line_utf8(null);
        ...
        _checkAuditFullRead() {
            ...
            do {
                [out, size] =
                    this._auditFullProcess_stream.read_line_utf8(null);
        ...
        checkShowChecking() {
            if (
                ...
                if (DISABLE_PARSING) {
                    ...
                    this._updateStatus(
                        this._updateList.filter(function (line) {
                            return RE_UpdateLine.test(line);
                        }).length
                    );
                }
        ...
        _showNotification(title, message) {
            ...
            this._notification.gicon = this._getCustIcon(
                'arch-updates-symbolic'
            );
            this._notification.addAction(_('Update now'), () => {
                this._updateNow();
            });
